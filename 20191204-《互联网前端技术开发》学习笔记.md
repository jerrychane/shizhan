...
document 对象还有一个 Cookie 属性，可以获得当前页面的 Cookie, Cookie 是由服务器发送的 key-value 标识符，因为 HTTP 协议是无状态，所以服务器需要区分是哪个用户发过来的请求，Cookie 的作用由此产生。当一个用户成功登录后，服务器发送一个 Cookie 给浏览器，如 user = jerrychane,
